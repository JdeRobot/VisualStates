...
'''
    ...
    def generateGlobalNamespace(self, globalNamespaceStr, projectName):
        ...
        # User defined Global Functions and Variables.
        if(len(self.globalNamespace.variables) > 0):
            for varLine in self.globalNamespace.variables.split('\n'):
                globalNamespaceStr.append('\t\t' + varLine + '\n')
            globalNamespaceStr.append('\n')

        if(len(self.globalNamespace.functions) > 0):
            for funcLine in self.globalNamespace.functions.split('\n'):
...
'''
        ...
        for state in self.getAllStates():
            mainStr.append('\tstate' + str(state.id) + ' = State' + str(state.id) +
                           '(' + str(state.id) + ', ' + str(state.initial) + ', globalNamespace, namespace'+ str(state.getNamespace().getID()) + ', '
